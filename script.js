...
const INDICATORS = [
  { key: 'shakeout',   label: '⚡ 극초기 개미털기(Shakeout)', weight: 18 },
  { key: 'cmf',        label: 'CMF 자금유입 (기관매집)',    weight: 16 },
  { key: 'rvol',       label: 'RVOL 상대거래량 급증',       weight: 14 },
  { key: 'up_down_vol', label: '상승/하락 거래량 우위',     weight: 12 },
  { key: 'obv',        label: 'OBV 추세 상승 전환',         weight: 12 },
  { key: 'ma_20_60',   label: 'MA 20/60 골든크로스',       weight: 12 },
  { key: 'bollinger',  label: '볼린저밴드 수축 후 돌파',    weight: 12 },
  { key: 'macd',       label: 'MACD 골든크로스',           weight: 10 },
...
// ============================================================
// [개선 1] 안정성: 지수 백오프 재시도 + 429/오류 구분 + 실패 카운트
// ============================================================
async function fetchViaProxy(targetUrl, { tries = 3, label = '' } = {}) {
  ...
  for (let i = 0; i < tries; i++) {
    ...
    try {
      ...
      lastErr = e;
      await sleep(200 * Math.pow(2, i)); // 200ms, 400ms, 800ms...
    }
  }
  failCount++;
...
// ============================================================
// [개선 2] SPAC/우선주 필터 정교화 — "trust"/"fund" 단독으로는 제외하지 않고
// SPAC 특화 조합(acquisition/blank check/spac 등)만 정밀 차단
// ============================================================
function isCommonStock(item) {
  ...
  // SPAC 특화 조합 (블록체크/인수목적회사) — "trust"/"fund"/"corp" 단독으로는 통과시킴
  const spacPatterns = [
...
async function fetchExchangeList(exchangeParam) {
  ...
  const json = await fetchViaProxy(target, { label: `${exchangeParam} 종목 리스트` });
  const rows = json && json.data && json.data.table && Array.isArray(json.data.table.rows)
...
// ============================================================
// [개선 3] 거래소 유니버스 캐싱 (sessionStorage, TTL 1시간)
// ============================================================
  ...
  for (let i = n - 4; i < n; i++) {
    ...
    if (isHammer || isSpringTrap) {
      return {
        ...
        shakeoutIndex: i,
        shakeoutPrice: b.low,
        type: isHammer ? '아래꼬리 투매흡수' : '지지선 훼이크 반등'
      };
    }
  }
...
function renderHudSvgChart(bars, ma20, ma60, bbUpper, bbLower, recentLow, shakeoutInfo, macdLine, macdSig, rsi9, rsi14) {
  ...
  const volBars = sliceBars.map((b, i) => {
    ...
    return `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="4" height="${vH.toFixed(1)}" fill="${isUp ? 'rgba(31,200,115,0.22)' : 'rgba(239,74,82,0.18)'}" />`;
  }).join('');
  ...
  if (shakeoutInfo && shakeoutInfo.status === 'crossed' && shakeoutInfo.shakeoutIndex !== undefined) {
    ...
    if (localIdx >= 0 && localIdx < n) {
      ...
      shakeoutMarker = `
        <circle cx="${sX}" cy="${sY}" r="6" fill="none" stroke="var(--shakeout)" stroke-width="1.8" />
        <circle cx="${sX}" cy="${sY}" r="3" fill="var(--shakeout)" />
  ...
  els.hudDynamicSvg.innerHTML = `
    ...
    <line x1="${PAD_X}" y1="${H*0.25}" x2="${W-PAD_X}" y2="${H*0.25}" stroke="rgba(255,255,255,0.04)" stroke-width="1" />
    <line x1="${PAD_X}" y1="${H*0.5}" x2="${W-PAD_X}" y2="${H*0.5}" stroke="rgba(255,255,255,0.04)" stroke-width="1" />
    <line x1="${PAD_X}" y1="${H*0.75}" x2="${W-PAD_X}" y2="${H*0.75}" stroke="rgba(255,255,255,0.04)" stroke-width="1" />
    ...
    ${ma20Pts ? `<polyline fill="none" stroke="var(--gold)" stroke-width="1.6" stroke-linecap="round" points="${ma20Pts}" />` : ''}
    <polyline fill="none" stroke="var(--cyan)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" points="${pricePts}" />
    ${macdMiniPts ? `<polyline fill="none" stroke="rgba(232,184,75,0.4)" stroke-width="1" stroke-dasharray="2,2" points="${macdMiniPts}" />` : ''}
    ${rsiMiniPts ? `<polyline fill="none" stroke="rgba(192,132,252,0.4)" stroke-width="1" points="${rsiMiniPts}" />` : ''}
    ${shakeoutMarker}
...
function analyzeStock(meta, bars, minDollarVol, onHudUpdate) {
  ...
  if (rangeRecent < 0.015) {
    return { dropped: true, reason: `스팩/거래정지주 의심 (20일 변동폭 ${(rangeRecent*100).toFixed(1)}% < 1.5%)` };
  }
  ...
  if (last < ma20[n - 1] * 0.94) {
    return { dropped: true, reason: `20일선 이탈 (-6% 이상)` };
  }
...
function cacheEls() {
  ['minPrice','maxPrice','minDollarVol','mktNasdaq','mktNyse','mktAmex','modeAuto','modeCustom','customTickerRow','customTickers',
   'tossWhitelist','concurrencySel','useCache',
   'minSignals','minBottom','scanBtn','resultsEmpty','resultsList','resultsCount','legendList','clock',
   'scannerOverlay','overlayStopBtn','overlayProgressBar','overlayProgressPct','overlayProgressCount',
   'hudFailBanner','hudFailText',
...
function updateFailBanner() {
  ...
  els.hudFailText.textContent = `⚠ 요청 실패/스킵 ${failCount}건 발생 (네트워크·레이트리밋). 결과가 적으면 동시 처리 수를 낮추고 재시도하세요.`;
}
...
function renderResults(list) {
  ...
  if (!list.length) {
    ...
    els.resultsEmpty.textContent = failCount > 0
      ? `조건을 만족하는 종목을 찾지 못했습니다. (요청 실패 ${failCount}건 — 네트워크 문제일 수 있어요)`
      : '조건을 만족하는 보통주를 찾지 못했습니다. 조건을 완화해보세요.';
    return;
  }
  ...
  list.forEach((r, idx) => {
    ...
    const tags = r.triggered
      ...
      .map((t) => {
        ...
        const tagClass = isShake ? 'tag--shake' : t.status === 'crossed' ? 'tag--crossed' : 'tag--imminent';
    ...
    const reasons = [];
    if (r.hasShakeout) {
      reasons.push(`<b style="color:var(--shakeout)">[⚡개미털기 포착]</b> 지지선 일시 이탈 투매 유도 후 +${r.distFromLowPct.toFixed(1)}% 이내 막 복귀`);
    }
    reasons.push(`최근 20일 변동폭 <b>${r.rangeRecentPct.toFixed(1)}%</b> 수렴 상태 (저점 지지)`);
    reasons.push(r.higherLow ? '최근 저가가 높아지는 <b>Higher-Low 구조</b> 형성' : '저점 지지력 유지 중');
    reasons.push(`매집·기술신호 <b>${r.triggeredCount}개 포착</b> (종합점수 ${r.totalScore.toFixed(1)}/100, 참고용 추정치)`);
...
async function buildUniverse(minPrice, maxPrice) {
  ...
  for (const ex of wanted) {
    ...
    if (useCache) {
      ...
      if (cached) {
        appendHudLog(`${EXCHANGES[ex].label} 캐시된 유니버스 사용 (${cached.length}개)`);
        universe = universe.concat(cached);
        continue;
      }
    }

    appendHudLog(`${EXCHANGES[ex].label} 보통주 유니버스 로드 중...`);
    try {
      ...
      appendHudLog(`[오류] ${EXCHANGES[ex].label} 유니버스 로드 실패 — ${e.message}`, 'error');
    }
  }
  ...
  if (whitelist) {
    ...
    appendHudLog(`토스 화이트리스트 적용: ${before}개 → ${universe.length}개로 축소`);
  }
...
async function runScan() {
  ...
  appendHudLog('9대 지표 통합 퀀트 스캐닝 엔진 가동 시작');

  try {
    const candidates = await buildUniverse(minPrice, maxPrice);

    if (!candidates.length) {
      appendHudLog('스캔 대상 유니버스가 비어 있습니다.');
      await sleep(800);
      renderResults([]);
      return;
    }

    appendHudLog(`유니버스 필터 통과: ${candidates.length}개사 정밀 분석 시작 (동시 ${CONCURRENCY}개)`);
        ...
        try {
          ...
          if (bars && bars.length) {
            const res = analyzeStock(
              ...
              (info) => {
                ...
                renderHudSvgChart(
                  ...
                  info.macdLine, info.macdSig, info.rsi9, info.rsi14
                );
                ...
                els.hudUpDownVal.className = `sub-metric-val ${info.upDownRatio >= 1.2 ? 'is-gc' : ''}`;
              }
            );
            ...
            if (res && !res.dropped) {
              analyzed.push(res);
              if (res.hasShakeout) {
                appendHudLog(`[⚡개미털기] ${c.code} (${c.name}) - 지지선 훼이크 복귀 포착! 점수: ${res.totalScore.toFixed(0)}점`, 'shake');
              } else {
    ...
    appendHudLog(`정밀 분석 완료 (실패 ${failCount}건) — 원래 화면으로 복귀 중…`);
    await sleep(400);
    ...
    if (e.message !== 'SCAN_ABORTED') {
      console.error(e);
      appendHudLog(`[치명적 오류] ${e.message}`, 'error');
