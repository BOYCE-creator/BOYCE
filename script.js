...
// [펌프앤덤프 / 시체 차트 감지 엔진]
// -------------------------------------------------------------
function detectPumpAndDumpDeadChart(bars, closes, lastPrice) {
  const n = closes.length;
  if (n < 60) return { isDead: false };

  const allHighs = bars.map(b => b.high);
  const maxHigh1Y = Math.max(...allHighs);
  const maxIdx1Y = allHighs.indexOf(maxHigh1Y);
  const dropFrom1YHigh = (lastPrice - maxHigh1Y) / (maxHigh1Y || 1);

  // 1. 최고점 대비 -65% 이상 폭락한 뒤 복구되지 못한 악성 낙폭 과대주 차단
  if (dropFrom1YHigh <= -0.65) {
    return {
      isDead: true,
      reason: `1년 최고점($${maxHigh1Y.toFixed(2)}) 대비 ${(dropFrom1YHigh * 100).toFixed(1)}% 대폭락 (악성 시체차트)`
    };
  }

  // 2. 단기 펌프 앤 덤프 폭락형 (특정 구간 급등 후 수직 낙하)
  const lookbackRecent = Math.min(90, n);
  ...
  if (spikeRatio >= 2.5 && dropFromRecentPeak <= -0.55 && barsSincePeak <= 60) {
    return {
      isDead: true,
      reason: `최근 단기 급등 후 수직 폭락 (${(dropFromRecentPeak * 100).toFixed(1)}% 하락 설거지 패턴)`
    };
  }
...
// -------------------------------------------------------------
// [가산점 1: 정밀 매물대 목표가 연산기 (비정상 스파이크 왜곡 방지)]
// -------------------------------------------------------------
function calculateTargetPriceAndBonus(bars, closes, lastPrice) {
  ...
  const isOverheatedTop = (ret5D >= 0.18);
  ...
  for (let i = 2; i < recentSlice.length - 2; i++) {
    ...
    if (
      ...
      // 주가 대비 3% 이상 ~ 200% 이하의 유효 저항선만 수집 (단발성 10배 폭등 침 왜곡 방지)
  ...
  for (const sh of swingHighs) {
    let matchedCluster = clusters.find(c => Math.abs((sh.price - c.avgPrice) / c.avgPrice) <= 0.03);
    if (matchedCluster) {
...
// -------------------------------------------------------------
// [가산점 2: NEOV형 세력 순환 매집 파동 연산기]
// -------------------------------------------------------------
function analyzeAccumulationWavePattern(bars, closes, lastPrice) {
  const n = closes.length;
  if (n < 60) return { isWavePattern: false, waveBonusScore: 0, waveRatio: 0, pulseCount: 0, desc: '데이터 부족' };

  const testLen = Math.min(150, n);
  const sliceBars = bars.slice(n - testLen);
  const sliceVolumes = sliceBars.map(b => b.volume);
  const volMa20 = sma(sliceVolumes, 20);

  const pulses = [];
  for (let i = 2; i < sliceBars.length - 2; i++) {
    const b = sliceBars[i];
    const avgV = volMa20[i] || 1;
    const volSurge = b.volume >= avgV * 1.5;
    const dayRange = (b.high - b.low) / (b.low || 1);
    const upperTail = (b.high - Math.max(b.open, b.close)) / (b.low || 1);

    if (volSurge && (dayRange >= 0.06 || upperTail >= 0.035)) {
      pulses.push({ index: i, high: b.high, low: b.low, price: b.close });
    }
  }

  if (pulses.length < 2) {
    return { isWavePattern: false, waveBonusScore: 0, waveRatio: 0, pulseCount: pulses.length, desc: '순환 파동 미발견' };
  }

  const baseLows = sliceBars.map(b => b.low);
  const waveFloor = Math.min(...baseLows);
  const waveCeil = Math.max(...pulses.map(p => p.high));
  const channelHeight = waveCeil - waveFloor;

  if (channelHeight <= 0) return { isWavePattern: false, waveBonusScore: 0, waveRatio: 0, pulseCount: 0, desc: '채널 형성 실패' };

  const waveRatio = clamp((lastPrice - waveFloor) / channelHeight, 0, 1);
  let waveBonusScore = 0;
  let wavePosDesc = '';

  if (waveRatio <= 0.22) {
    waveBonusScore = 15;
    wavePosDesc = `파동 최하단 ${(waveRatio * 100).toFixed(0)}% 위치 (만점 지지)`;
  } else if (waveRatio <= 0.38) {
    waveBonusScore = 12;
    wavePosDesc = `파동 저점권 ${(waveRatio * 100).toFixed(0)}% 위치 (우수 지지)`;
  } else if (waveRatio <= 0.50) {
    waveBonusScore = 7;
    wavePosDesc = `파동 중하단 ${(waveRatio * 100).toFixed(0)}% 위치 (중립)`;
  } else if (waveRatio <= 0.70) {
    waveBonusScore = 3;
    wavePosDesc = `파동 중상단 ${(waveRatio * 100).toFixed(0)}% 위치 (상승 진행)`;
  } else {
    waveBonusScore = 0;
    wavePosDesc = `⚠️ 파동 고점권 ${(waveRatio * 100).toFixed(0)}% 위치 (추격 금지)`;
  }
  ...
  return {
    ...
    waveCeil,
    wavePosDesc,
    desc: `순환 매집파동 ${pulses.length}회 관측 · ${wavePosDesc}`
  };
}
...
function detectChoppyNoiseChart(bars, closes) {
  ...
  for (const b of testBars) {
    ...
    if (range > 0 && (body / range < 0.35 || rangePct < 1.0)) {
      smallBodyCount++;
    }
  }

  const avgRangePct = totalRangePct / testBars.length;
  if (smallBodyCount >= 18 || avgRangePct < 1.2) {
    return true;
  }
  return false;
}

function detectBadChartPatterns(bars, closes, ma20, ma60, ma200) {
  const n = closes.length;
  ...
  if (n >= 60) {
    ...
    const isFullBearishMA = (cMa20 < cMa60) && (cMa60 < cMa200) && (last < cMa60 * 0.95);
    const winLong = Math.min(120, n);
    const maxHighLong = Math.max(...closes.slice(n - winLong));
    const dropFromHigh = (last - maxHighLong) / (maxHighLong || 1);

    const ma60Prev20 = ma60[Math.max(0, n - 20)] || cMa60;
    const ma60Slope = (cMa60 - ma60Prev20) / (ma60Prev20 || 1);

    if (isFullBearishMA && ma60Slope < -0.035) {
      return { bad: true, reason: `장기 우하향 역배열 (MA60 기울기 ${(ma60Slope * 100).toFixed(1)}%)` };
    }
    if (dropFromHigh < -0.50 && last < cMa60) {
      return { bad: true, reason: `장기 급락 추세 미복구 (고점대비 ${(dropFromHigh * 100).toFixed(1)}%)` };
    }
  }

  if (n >= 40) {
    const winFlat = Math.min(60, n);
    const sliceFlat = closes.slice(n - winFlat);
    const mean = sliceFlat.reduce((a, b) => a + b, 0) / winFlat;
    const variance = sliceFlat.reduce((s, v) => s + (v - mean) ** 2, 0) / winFlat;
    const std = Math.sqrt(variance);
    const cv = (std / (mean || 1)) * 100;

    const sliceHigh = Math.max(...sliceFlat);
    const sliceLow = Math.min(...sliceFlat);
    const maxSpanPct = ((sliceHigh - sliceLow) / (sliceLow || 1)) * 100;

    if (cv < 1.4 || maxSpanPct < 3.0) {
      return { bad: true, reason: `장기 무반응 일자 횡보 (변동계수 ${cv.toFixed(2)}% < 1.4%)` };
    }
  }
...
function renderHudSvgChart(bars, ma20, ma60, ma200, bbUpper, bbLower, recentLow, targetPrice, shakeoutInfo, macdLine, rsi9) {
  ...
  const sliceMa200 = (ma200 || []).slice(startIdx);
  const sliceBbU = (bbUpper || []).slice(startIdx);
  const sliceBbL = (bbLower || []).slice(startIdx);

  const validVals = [...closes, ...sliceMa20, ...sliceMa60, ...sliceMa200, ...sliceBbU, ...sliceBbL, recentLow, targetPrice].filter(v => Number.isFinite(v) && v > 0);
  ...
  const ma200Pts = sliceMa200.map((m, i) => !Number.isFinite(m) ? null : `${getX(i).toFixed(1)},${getYPrice(m).toFixed(1)}`).filter(Boolean).join(' ');
  const bbUPts = sliceBbU.map((u, i) => !Number.isFinite(u) ? null : `${getX(i).toFixed(1)},${getYPrice(u).toFixed(1)}`).filter(Boolean).join(' ');
  const bbLPts = sliceBbL.map((l, i) => !Number.isFinite(l) ? null : `${getX(i).toFixed(1)},${getYPrice(l).toFixed(1)}`).filter(Boolean).join(' ');
  ...
  els.hudDynamicSvg.innerHTML = `
    ...
    <line x1="${PAD_X}" y1="${baseLineY}" x2="${W-PAD_X}" y2="${baseLineY}" stroke="#ff4772" stroke-width="1.2" stroke-dasharray="3,3" />
    <text x="${W-PAD_X-4}" y="${baseLineY - 3}" fill="#ff4772" font-family="JetBrains Mono" font-size="8" text-anchor="end">SUPPORT $${recentLow.toFixed(2)}</text>
    ...
    ${bbUPts ? `<polyline fill="none" stroke="rgba(255,107,129,0.2)" stroke-width="1" stroke-dasharray="2,2" points="${bbUPts}" />` : ''}
    ${bbLPts ? `<polyline fill="none" stroke="rgba(255,107,129,0.2)" stroke-width="1" stroke-dasharray="2,2" points="${bbLPts}" />` : ''}
    ${ma200Pts ? `<polyline fill="none" stroke="#3b82f6" stroke-width="1.0" opacity="0.6" stroke-dasharray="3,3" points="${ma200Pts}" />` : ''}
...
function analyzeStockPhase1(meta, bars, filterOpts, isCustomMode, onHudUpdate) {
  ...
  // 1. [필터 1] 펌프앤덤프 / 시체 차트 원천 차단
  const deadCheck = detectPumpAndDumpDeadChart(bars, closes, last);
  ...
  const badCheck = detectBadChartPatterns(bars, closes, ma20, ma60, ma200);
  ...
  if (ma20GapPct < -10) {
    if (!isCustomMode) return { dropped: true, reason: `20일선 과도한 이탈 (${ma20GapPct.toFixed(1)}%)` };
    ma20TrendPenalty = 6;
  } else if (ma20GapPct < -6) {
    ma20TrendPenalty = 4;
  } else if (ma20GapPct < -3) {
    ma20TrendPenalty = 2;
  }
  ...
  // 4. 순환 매집 파동 가산점 연산
  const waveInfo = analyzeAccumulationWavePattern(bars, closes, last);

  const isHealthyLongTrend = curMa200 > 0 && last >= curMa200 * 0.93;
  ...
  if (ret5 > 0.15 || targetInfo.isOverheatedTop) {
...
function renderLegend() {
  els.legendList.innerHTML = [
    ...
    `<li><b style="color:var(--wave-amber)">🌊 NEOV형 순환 매집파동 저점 가산</b><span style="color:var(--wave-amber)">최대 +15점</span></li>`
  ].join('');
}
...
function renderResults(list) {
  ...
  top24List.forEach((r, idx) => {
    ...
    if (r.waveInfo?.waveBonusScore > 0) {
      tags.unshift(`<span class="tag tag--wave">🌊 파동바닥 (${r.waveInfo.waveRatio.toFixed(0)}%) +${r.waveInfo.waveBonusScore}점</span>`);
    }

    const reasons = [];
    if (r.waveInfo?.waveBonusScore > 0) {
      reasons.push(`<b style="color:var(--wave-amber)">[🌊 순환 매집파동 가산 +${r.waveInfo.waveBonusScore}점]</b> ${r.waveInfo.desc}`);
    }
    if (r.targetBonusScore > 0) {
    ...
    card.innerHTML = `
      ...
      <div class="card__score-grid">
        ...
        <span class="score-pill score-pill--wave" title="순환 매집파동 가산점">파동바닥 <b>+${r.waveInfo?.waveBonusScore || 0}</b></span>
      </div>
    ...
    card.addEventListener('click', () => {
      ...
      renderHudSvgChart(r.bars, r.ma20, r.ma60, r.ma200, r.bbUpper, r.bbLower, r.recentLow, r.targetPrice, r.shakeoutInfo, r.macdLine, r.rsi9);
...
async function runScan() {
  ...
  appendHudLog(`Phase 1: 펌프앤덤프/설거지주 필터링 및 NEOV 파동 퀀트 엔진 가동`);
          ...
          if (bars && bars.length) {
                ...
                if (now - lastHudRenderTime > 100) {
                  ...
                  requestAnimationFrame(() => {
                    ...
                    renderHudSvgChart(
                      ...
                      info.shakeoutInfo, info.macdLine, info.rsi9
                    );
            ...
            if (res && !res.dropped) {
              ...
              if (res.waveInfo?.waveBonusScore >= 12) {
                appendHudLog(`[🌊순환파동] ${c.code} (${c.name}) - 파동바닥 지지 (+${res.waveInfo.waveBonusScore}점)`, 'wave');
              } else {
